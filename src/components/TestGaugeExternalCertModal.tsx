...
import {
  loadTestGaugeCalibrationEvents,
  recordExternalTestGaugeCalibration,
...
import type { TestGauge, TestGaugeCalibrationEvent } from '../types/testGauge'
...
export function TestGaugeExternalCertModal({
  gauge,
  initialTab = 'new',
  onClose,
  onSaved,
  ...
  useEffect(() => {
    ...
    void loadTestGaugeCalibrationEvents(gauge.id).then(({ events, error }) => {
      if (cancelled) return
  ...
  const save = async () => {
    ...
    setSaving(true)
    const { error } = await recordExternalTestGaugeCalibration(gauge, {
      calibratedAt,
      nextDueAt,
  ...
  return (
    <div className="modal-overlay" role="presentation" onClick={onClose}>
      <div
        ...
        role="dialog"
        aria-labelledby="gauge-ext-cert-title"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="modal-header-with-close">
          <div className="modal-header-text">
            <h2 id="gauge-ext-cert-title">Upload certificate — {gauge.gauge_number}</h2>
            <p className="modal-meta">
              {gauge.manufacturer ?? '—'} · {gauge.gauge_type ?? '—'}
            </p>
          </div>
        ...
        {tab === 'new' ? (
          <>
            <p className="placeholder-copy resources-hint">
              Test gauges are calibrated by an outside lab. Enter the new certificate details — the current
              calibration is archived automatically.
            </p>

            {willArchive ? (
              <p className="tool-recal-gauge-warn">
                Current record will be archived
                {gauge.certificate_number ? ` (cert # ${gauge.certificate_number})` : ''}
                {gauge.last_calibration_date
                  ? ` · calibrated ${formatShortDate(gauge.last_calibration_date)}`
                  : ''}
                {gauge.next_calibration_date
                  ? ` · due ${formatShortDate(gauge.next_calibration_date)}`
                  : ''}
                .
              </p>
            ) : null}
          ...
          <div className="tool-recal-history">
            {historyLoading ? (
              ...
              <p className="placeholder-copy">No archived calibration certificates yet for this gauge.</p>
            ) : (
              <table className="tool-recal-table">
                ...
                <tbody>
                  {history.map((event) => {
                    const url = testGaugeCertificateUrl(event.certificate_storage_path)
                    return (
                      <tr key={event.id}>
