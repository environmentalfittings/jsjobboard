...
function formToPayload(form: QualityIncrFormState, options?: { includeFiveWhys?: boolean }) {
  ...
  const synced = syncIncrStatusWithApprovals(form)
  return {
    status: synced.status,
    customer_name: emptyToNull(synced.customer_name),
    date_rejected: emptyToNull(synced.date_rejected),
    wo_so: emptyToNull(synced.wo_so),
    part_description: emptyToNull(synced.part_description),
    employee_name: emptyToNull(synced.employee_name),
    dept_responsible: emptyToNull(synced.dept_responsible),
    work_cell: emptyToNull(synced.work_cell),
    item: emptyToNull(synced.item),
    nonconformance_details: emptyToNull(synced.nonconformance_details),
    discrepancy_description: emptyToNull(synced.discrepancy_description),
    ...(includeFiveWhys ? { five_whys: normalizeFiveWhys(synced.five_whys) } : {}),
    disposition: synced.disposition || null,
    final_disposition: emptyToNull(synced.final_disposition),
    labor_cost: emptyToNull(synced.labor_cost),
    material_cost: emptyToNull(synced.material_cost),
    root_cause_corrective_action: emptyToNull(synced.root_cause_corrective_action),
    qc_approval_name: emptyToNull(synced.qc_approval_name),
    qc_approval_date: emptyToNull(synced.qc_approval_date),
    initiator_name: emptyToNull(synced.initiator_name),
    initiator_date: emptyToNull(synced.initiator_date),
    final_approval_name: emptyToNull(synced.final_approval_name),
    final_approval_date: emptyToNull(synced.final_approval_date),
    customer_signature_required: Boolean(synced.customer_signature_required),
    customer_signature_date: emptyToNull(synced.customer_signature_date),
    requires_corporate_ncr: Boolean(synced.requires_corporate_ncr),
    notes: emptyToNull(synced.notes),
